...
Plugin URI: http://rxa.li/mymail?utm_campaign=wporg&utm_source=Google+Analytics+for+MyMail
License: GPLv2
...
class MyMailGoogleAnalytics {
	...
	public function init() {
		...
		if ( function_exists( 'mailster' ) ) {
			...
			add_action( 'admin_notices', function(){
				...
				$name = 'Google Analytics for MyMail';
				$slug = 'mailster-google-analytics/mailster-google-analytics.php';
	...
	public function redirect_to( $target, $campaign_id ) {
		...
		if ( $target_domain !== $site_domain ) { return $target;
		}
	...
	public function add_meta_boxes() {
		...
		if ( mymail_option( 'ga_campaign_based' ) ) {
			add_meta_box( 'mymail_ga', 'Google Analytics', array( $this, 'metabox' ), 'newsletter', 'side', 'low' );
		}
	}
	...
	<table class="form-table">
		...
		<tr valign="top">
			...
			<td><label><input type="hidden" name="mymail_options[ga_campaign_based]" value=""><input type="checkbox" name="mymail_options[ga_campaign_based]" value="1" <?php checked( mymail_option( 'ga_campaign_based' ) )?>> <?php _e( 'allow campaign based variations of these values', 'mymail_ga' ); ?></label><p class="description"><?php _e( 'adds a metabox on the campaign edit screen to alter the values for each campaign', 'mymail_ga' ); ?></p></td>
		</tr>
	...
	public function notice() {
		$msg = sprintf( __( 'You have to enable the %s to use the Google Analytics Extension!', 'mymail_ga' ), '<a href="http://rxa.li/mymail?utm_campaign=wporg&utm_source=Google+Analytics+for+MyMail">MyMail Newsletter Plugin</a>' );
	?>
	...
	public function activate() {
		...
		if ( function_exists( 'mymail' ) ) {
			...
			if ( ! mymail_option( 'ga_id' ) ) {
				mymail_notice( sprintf( __( 'Please enter your Web Property ID on the %s!', 'mymail_ga' ), '<a href="options-general.php?page=newsletter-settings&mymail_remove_notice=google_analytics#ga">Settings Page</a>' ), '', false, 'google_analytics' );
			}
		}
